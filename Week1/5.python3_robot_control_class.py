...
class Robo:
    def __init__(self, robot_control):
        ...
        self.laser1 = self.robotcontrol.get_laser(360)
    ...
    def robotmove(self):
        ...
        print("I am too close to the wall... ")

    def robotturn(self):
        distance_right = self.robotcontrol.get_laser(180)
        distance_left = self.robotcontrol.get_laser(540)
        print("right", distance_right, "left", distance_left)
        if distance_right > distance_left:
            while self.laser1 < 1:
                self.robotcontrol.turn(self.robotturn_clockwise, self.robotturn_speed, self.robotturn_time)
                self.laser1 = self.robotcontrol.get_laser(360)
                print("right: distance: ", self.laser1)
        else:
            while self.laser1 < 1:
                self.robotcontrol.turn("aclockwise", self.robotturn_speed, self.robotturn_time)
                self.laser1 = self.robotcontrol.get_laser(360)
                print("left: distance: ", self.laser1)

        self.robotcontrol.stop_robot()
        print("I have turned, let's go... ")


if __name__ == '__main__':
    robotcontrol_object = RobotControl()
    robo = Robo(robotcontrol_object)

    while not robotcontrol_object.ctrl_c:
        robo.robotmove()
        robo.robotturn()